...
no_overview =
  Repo.one(from m in Movie, where: is_nil(m.overview) or m.overview == "", select: count(m.id))

no_release_date = Repo.one(from m in Movie, where: is_nil(m.release_date), select: count(m.id))

no_runtime =
  Repo.one(from m in Movie, where: is_nil(m.runtime) or m.runtime == 0, select: count(m.id))

no_genres =
  Repo.one(
    from m in Movie,
      left_join: mg in "movie_genres",
      on: m.id == mg.movie_id,
...
IO.puts(
  "Movies without overview: #{no_overview} (#{Float.round(no_overview / movie_count * 100, 1)}%)"
)

IO.puts(
  "Movies without release date: #{no_release_date} (#{Float.round(no_release_date / movie_count * 100, 1)}%)"
)

IO.puts(
  "Movies without runtime: #{no_runtime} (#{Float.round(no_runtime / movie_count * 100, 1)}%)"
)
...
Enum.each(low_quality_movies, fn movie ->
  IO.puts(
    "  - #{movie.title}: popularity=#{movie.popularity}, votes=#{movie.vote_count}, poster=#{movie.has_poster}"
  )
end)
...
IO.puts(
  "Total potentially low-quality movies: #{low_quality_count} (#{Float.round(low_quality_count / movie_count * 100, 1)}%)"
)
...
IO.puts(
  "People without profile photo: #{no_profile} (#{Float.round(no_profile / person_count * 100, 1)}%)"
)

IO.puts(
  "People with zero popularity: #{zero_popularity} (#{Float.round(zero_popularity / person_count * 100, 1)}%)"
)
...
IO.puts(
  "Movies with IMDb ID: #{with_imdb_id} (#{Float.round(with_imdb_id / movie_count * 100, 1)}%)"
)
...
tables =
  Repo.query!("SELECT tablename FROM pg_tables WHERE schemaname = 'public' ORDER BY tablename").rows

feature_tables =
  Enum.filter(tables, fn [name] ->
...
critical_missing = %{
  "Movies without any credits" =>
    Repo.one(
      from m in Movie,
        left_join: c in Credit,
        on: m.id == c.movie_id,
        where: is_nil(c.id),
        select: count(m.id, :distinct)
    ),
  "Movies without production companies" =>
    Repo.one(
      from m in Movie,
  ...
  "Movies without keywords" =>
    Repo.one(
      from m in Movie,
        left_join: mk in "movie_keywords",
        on: m.id == mk.movie_id,
