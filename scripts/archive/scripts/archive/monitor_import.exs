...
defmodule ImportMonitor do
  def check_status do
    ...
    # Calculate rates
    pages_processed =
      case Integer.parse(to_string(progress.last_page_processed || "0")) do
        {num, _} -> num
        :error -> 0
      end
    ...
    IO.puts(
      "  Movies: #{progress.our_total_movies} / #{progress.tmdb_total_movies} (#{progress.completion_percentage}%)"
    )
    ...
    IO.puts(
      "  ETA for 10k: ~#{Float.round((10_000 - progress.our_total_movies) / Kernel.max(import_rate, 1), 1)} minutes"
    )
    ...
    IO.puts(
      "  With OMDb data: #{with_omdb} (#{Float.round(with_omdb / Kernel.max(progress.our_total_movies, 1) * 100, 1)}%)"
    )
