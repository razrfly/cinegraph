...
if length(fallback_stats) > 0 do
  Enum.each(fallback_stats, fn stat ->
    ...
    IO.puts("     Attempts: #{total} | Success: #{success_rate}% | Avg Confidence: #{confidence}")
  end)
else
...
total_calls = Enum.sum(Enum.map(overall_stats, & (&1.total || 0)))
successful_calls = Enum.sum(Enum.map(overall_stats, & (&1.successful || 0)))
