...
partnerships = Repo.all(
  from c in Cinegraph.Collaborations.Collaboration,
  join: cd in Cinegraph.Collaborations.CollaborationDetail, on: cd.collaboration_id == c.id,
...
if length(partnerships) > 0 do
  ...
  # Get their IDs
  actor_id = Repo.one(from p in Cinegraph.Movies.Person, where: p.name == ^actor_name, select: p.id)
  director_id = Repo.one(from p in Cinegraph.Movies.Person, where: p.name == ^director_name, select: p.id)
