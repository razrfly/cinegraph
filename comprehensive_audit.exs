...
IO.puts(
  "  Full imports: #{full_imports} (#{Float.round(full_imports / max(total_movies, 1) * 100, 1)}%)"
)

IO.puts(
  "  Soft imports: #{soft_imports} (#{Float.round(soft_imports / max(total_movies, 1) * 100, 1)}%)"
)
...
# Movies quality
movies_with_posters =
  Repo.one(from m in Movie, where: not is_nil(m.poster_path), select: count(m.id))

movies_with_high_votes = Repo.one(from m in Movie, where: m.vote_count >= 25, select: count(m.id))

movies_with_high_popularity =
  Repo.one(from m in Movie, where: m.popularity >= 5.0, select: count(m.id))

IO.puts("\nMovie Quality (all movies):")

IO.puts(
  "  With posters: #{movies_with_posters} (#{Float.round(movies_with_posters / max(total_movies, 1) * 100, 1)}%)"
)

IO.puts(
  "  With ≥25 votes: #{movies_with_high_votes} (#{Float.round(movies_with_high_votes / max(total_movies, 1) * 100, 1)}%)"
)

IO.puts(
  "  With ≥5.0 popularity: #{movies_with_high_popularity} (#{Float.round(movies_with_high_popularity / max(total_movies, 1) * 100, 1)}%)"
)
...
if full_imports > 0 do
  full_no_poster =
    Repo.one(
      from m in Movie,
        where: m.import_status == "full" and is_nil(m.poster_path),
        select: count(m.id)
    )

  full_low_votes =
    Repo.one(
      from m in Movie, where: m.import_status == "full" and m.vote_count < 25, select: count(m.id)
    )

  full_low_pop =
    Repo.one(
      from m in Movie,
        where: m.import_status == "full" and m.popularity < 5.0,
        select: count(m.id)
    )
...
# People quality
people_with_photos =
  Repo.one(from p in Person, where: not is_nil(p.profile_path), select: count(p.id))

people_with_popularity =
  Repo.one(from p in Person, where: p.popularity >= 0.5, select: count(p.id))

IO.puts("\nPeople Quality:")

IO.puts(
  "  With photos: #{people_with_photos} (#{Float.round(people_with_photos / max(people_count, 1) * 100, 1)}%)"
)

IO.puts(
  "  With ≥0.5 popularity: #{people_with_popularity} (#{Float.round(people_with_popularity / max(people_count, 1) * 100, 1)}%)"
)
...
movie_companies_count =
  Repo.one(from mpc in "movie_production_companies", select: count(mpc.movie_id))
...
IO.puts(
  "  Production Companies: #{companies_count} total, #{movie_companies_count} movie associations"
)
...
IO.puts(
  "  Movies with genres: #{movies_with_genres}/#{full_imports} (#{Float.round(movies_with_genres / max(full_imports, 1) * 100, 1)}%)"
)

IO.puts(
  "  Movies with keywords: #{movies_with_keywords}/#{full_imports} (#{Float.round(movies_with_keywords / max(full_imports, 1) * 100, 1)}%)"
)
...
IO.puts(
  "Movies with TMDB data: #{movies_with_tmdb}/#{total_movies} (#{Float.round(movies_with_tmdb / max(total_movies, 1) * 100, 1)}%)"
)
...
IO.puts(
  "Movies with IMDb ID: #{movies_with_imdb_id}/#{total_movies} (#{Float.round(movies_with_imdb_id / max(total_movies, 1) * 100, 1)}%)"
)

IO.puts(
  "Movies with OMDb data: #{movies_with_omdb}/#{movies_with_imdb_id} with IMDb ID (#{Float.round(movies_with_omdb / max(movies_with_imdb_id, 1) * 100, 1)}%)"
)
...
if full_imports > movies_with_genres,
  do: issues = ["#{full_imports - movies_with_genres} full imports without genres" | issues]

if full_imports > movies_with_keywords,
  do: issues = ["#{full_imports - movies_with_keywords} full imports without keywords" | issues]

# Check collaborations
if credits_count > 0 and collab_count == 0,
  do: issues = ["No collaborations built despite having credits" | issues]

# Check external enrichment
if movies_with_imdb_id > movies_with_omdb,
  do:
    issues = [
      "#{movies_with_imdb_id - movies_with_omdb} movies with IMDb ID but no OMDb data" | issues
    ]
...
IO.puts(
  "□ Videos & Release Dates - ✓ Working (#{videos_count} videos, #{release_dates_count} dates)"
)

IO.puts("□ Quality Filtering - ✓ Working (#{soft_imports} soft imports)")

IO.puts(
  "□ Collaboration Tracking - #{if collab_count > 0, do: "✓ Working (#{collab_count} collaborations)", else: "✗ NO DATA"}"
)
