...
candidate_definitions = [
  ...
  %{
    ...
    "definition" => "Shrinks current score toward release/popularity/language/evidence cohort mean.",
  ...
  %{
    ...
    "definition" => "2+ lens display, confidence-adjusted sorting, cohort percentile tie-breaker.",
  ...
  for target <- target_sets, candidate <- candidate_keys do
    ...
    row =
      ...
      SELECT
        ...
        (SELECT positives FROM counts) AS usable_positives,
  ...
  SELECT
    ...
    COUNT(*) FILTER (
      WHERE evidence_regime = 'high_evidence'
        AND (validation_target_canonical_any OR validation_target_award_any)
        AND candidate_score IS NOT NULL
        AND candidate_score >= 2.0
...
#{B.md_table(candidate_definitions, [
  {"Candidate", "candidate"},
...
#{B.md_table(leaderboard, [
  ...
  {"Hidden positives", "hidden_positive_pct"},
  {"Visible false-low", "visible_false_low_pct"}
])}

## Current Vs Hybrid

#{B.md_table([
  Map.merge(current_canonical, %{"comparison" => "Canonical target / current"}),
  Map.merge(hybrid_canonical, %{"comparison" => "Canonical target / hybrid"}),
  Map.merge(current_award, %{"comparison" => "Award target / current"}),
  Map.merge(hybrid_award, %{"comparison" => "Award target / hybrid"})
], [
...
#{B.md_table(coverage_rows, [
  ...
  {"High conf", "high_confidence"},
  {"Medium conf", "medium_confidence"},
  {"Low conf", "low_confidence"},
  {"Insufficient conf", "insufficient_confidence"}
])}
...
#{B.md_table(threshold_rows, [
  {"Threshold", "threshold"},
  {"Visible rows", "visible_rows"},
  {"Visible %", "visible_pct"},
  {"Hidden known positives", "hidden_known_positives"},
  {"Hidden known-positive %", "hidden_known_positive_pct"},
  {"Visible known-positive false-low %", "visible_known_positive_false_low_pct"}
])}

## Product-Safety Checks

#{B.md_table(safety_rows, [
  {"Candidate", "candidate"},
  {"0-lens scoreable", "zero_lens_scoreable"},
  {"1-lens scoreable", "one_lens_scoreable"},
  {"0-1 lens medium/high conf", "low_lens_medium_or_high_confidence"},
  {"High-evidence positives not false-low", "high_evidence_positive_not_false_low"},
  {"Sparse positives visible false-low", "sparse_positive_visible_false_low"}
])}
...
#{B.md_table(high_risk_rows, [
  {"Risk cohort", "risk_cohort"},
  {"Candidate", "candidate"},
  {"Movies", "movies"},
...
#{B.md_table(cohort_rows, [
  {"Cohort", "cohort"},
  {"Bucket", "bucket"},
  {"Candidate", "candidate"},
  {"Movies", "movies"},
  {"Known positives", "known_positives"},
  {"Avg candidate score", "avg_candidate_score"},
  {"Hidden known-positive %", "hidden_known_positive_pct"},
  {"Visible false-low %", "visible_false_low_pct"}
])}
...
#{B.md_table(known_positive_failures, [
  {"Candidate", "candidate"},
  {"Movie", "title"},
  {"Year", "release_year"},
...
#{B.md_table(high_signal_non_targets, [
  {"Candidate", "candidate"},
  {"Movie", "title"},
  {"Year", "release_year"},
  {"Score", "candidate_score"},
