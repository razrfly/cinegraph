defmodule Cinegraph.Repo.Migrations.RecreateMovieScoreabilityViewWithFloatConfidence do
  use Ecto.Migration
  ...
  def down do
    execute "DROP VIEW IF EXISTS movie_scoreability_view"
    execute previous_view_sql()
  end

  defp create_view_sql do
    """
    CREATE VIEW movie_scoreability_view AS
    WITH score_base AS (
      SELECT
        m.id AS movie_id,
        m.title,
    ...
    SELECT
      *,
      ...
      CASE
        WHEN raw_cinegraph_score IS NOT NULL AND present_lens_count >= 2 THEN raw_cinegraph_score::numeric * evidence_confidence
        ELSE NULL
    ...
    scored AS (
      SELECT
        *,
        CARDINALITY(present_lens_labels) AS present_lens_count,
        CARDINALITY(missing_lens_labels) AS missing_lens_count,
        ROUND((CARDINALITY(present_lens_labels)::numeric / 6.0), 3) AS evidence_confidence,
        NULL::numeric AS cohort_percentile
      FROM score_base
    )
    SELECT
      *,
