defmodule Mix.Tasks.ImportCanonical do
  ...
  def run(args) do
    ...
    cond do
      ...
      true ->
        Mix.shell().error("Please specify --list, --all, or provide --list-id, --source-key, and --name")
        Mix.shell().info("\nUsage: mix import_canonical --list 1001_movies")
        Mix.shell().info("       mix import_canonical --list-id ls024863935 --source-key 1001_movies --name \"1001 Movies\"")
    ...
    case Map.get(predefined_lists(), list_key) do
      ...
      list_config ->
        ...
        if dry_run do
          ...
          result = Cinegraph.Cultural.CanonicalImporter.import_canonical_list(
            list_config.list_id,
