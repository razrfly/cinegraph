defmodule Mix.Tasks.Pqs do
      ...
      {min, _} ->
        case PersonQualityScore.calculate_all_person_scores(min) do
          {:ok, %{total: total, successful: successful}} ->
            Mix.shell().info(
              "✅ Batch calculation complete: #{successful}/#{total} people processed"
            )
  ...
  def run(["monitor"]) do
    ...
    Mix.shell().info(
      "  Fresh Scores (<7 days): #{freshness.fresh_scores} (#{freshness.fresh_percent}%)"
    )
    ...
    Enum.each(breakdown, fn role ->
      Mix.shell().info(
        "  #{role.role}: #{role.people_with_pqs}/#{role.total_people} (#{role.coverage_percent}%)"
      )
    end)
  end
  ...
  defp create_basic_test_module do
    ...
    case Cinegraph.Repo.one(from p in "people", limit: 1, select: p.id) do
      nil ->
        Mix.shell().info("⚠️  No people found in database for testing")
    ...
    case Cinegraph.Repo.one(from mc in "movie_credits", limit: 1, select: mc.person_id) do
      nil ->
        Mix.shell().info("⚠️  No movie credits found for testing")
    ...
    case Cinegraph.Repo.one(from m in "movies", limit: 1, select: m.id) do
      nil ->
        Mix.shell().info("⚠️  No movies found for testing")
