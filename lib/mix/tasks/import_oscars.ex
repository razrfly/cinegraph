defmodule Mix.Tasks.ImportOscars do
  ...
  defp import_single_year(year, options, dry_run) do
    Mix.shell().info(
      "#{if dry_run, do: "[DRY RUN] ", else: ""}Importing Oscar data for #{year}..."
    )
  ...
  defp import_year_range(range_str, options, dry_run) do
    case parse_year_range(range_str) do
      {:ok, start_year, end_year} ->
        Mix.shell().info(
          "#{if dry_run, do: "[DRY RUN] ", else: ""}Importing Oscar data for years #{start_year}-#{end_year}..."
        )
  ...
  defp show_year_preview(year) do
    ...
    Mix.shell().info(
      "  • Year #{year} (#{Number.Human.number_to_ordinal(ceremony_number)} Academy Awards)"
    )
