defmodule Cinegraph.Scrapers.ImdbCanonicalScraper do
  ...
  defp find_total_count_text(document) do
    ...
    count_selectors = [
      # Most specific - targets the exact location
      "[data-testid='list-page-mc-total-items'] .ipc-inline-list__item",
    ...
    if title_link do
      ...
      if imdb_id && title != "" do
        ...
        if tracks_awards do
          ...
          Logger.info(
            "   Enhanced result has extracted_awards: #{Map.has_key?(result, :extracted_awards)}"
          )
  ...
  defp parse_ipc_item(item, position, tracks_awards) do
    ...
    title_link =
      Floki.find(item, ".ipc-title-link-wrapper a[href*='/title/tt']") |> List.first() ||
    ...
    if title_link do
      ...
      title =
        if title_text != "",
          do: clean_title(title_text),
          else: Floki.text(title_link) |> String.trim() |> clean_title()
      ...
      if imdb_id && title != "" do
        ...
        if tracks_awards do
          ...
          Logger.info(
            "   IPC: Enhanced result has extracted_awards: #{Map.has_key?(result, :extracted_awards)}"
          )
  ...
  defp extract_movie_from_title_link(link, position) do
    ...
    if imdb_id && title != "" do
      %{
        imdb_id: imdb_id,
        title: title,
        # Year extraction would need parent context
        year: nil,
        position: position
      }
    else
  ...
  defp extract_year_from_ipc_item(item) do
    ...
    year_selectors = [
      ...
      "span.sc-479faa3c-8",
      # Check all spans for year pattern
      "span"
    ]
  ...
  defp extract_enhanced_lister_data(item) do
    ...
    award_text =
      Enum.find_value(award_element_selectors, fn selector ->
    ...
    if length(award_matches) > 0 do
      ...
      award_lines =
        ...
        |> Enum.filter(fn line ->
          ...
          # Not ratings
          # Not runtime
          # Not ratings like R, PG
          # Not credits
          String.length(trimmed) < 100 &&
            !String.match?(trimmed, ~r/^\d+\.\d+$/) &&
            !String.match?(trimmed, ~r/^\d+h?\s*\d*m?$/) &&
            !String.match?(trimmed, ~r/^[A-Z]+$/) &&
            !String.match?(trimmed, ~r/Director:|Stars:/) &&
  ...
  defp parse_award_text_to_structured(award_text)
       when is_binary(award_text) and award_text != "" do
        ...
        case Regex.run(~r/^([^(]+?)\s+winner(?:\s*\(([^)]+)\))?\s*\.?$/i, trimmed) do
          [_, award_name, category] ->
            result = %{
              ...
              award_category:
                if(category && category != "", do: String.trim(category), else: nil),
              award_year: nil,
              raw_text: trimmed
            }
          ...
          _ ->
            ...
            if Regex.match?(
                 ~r/^(Palme d'Or|Grand Prix|Jury Prize|Caméra d'Or|Camera d'Or|Best Director|Best Actor|Best Actress)$/i,
                 trimmed
               ) do
              result = %{
              ...
              case Regex.run(~r/Won\s+(.+?)(?:\s*\(([^)]+)\))?\s*\.?$/i, trimmed) do
                ...
                _ ->
                  ...
                  if Regex.match?(~r/(Festival|Awards?|Prize|Competition)/i, trimmed) do
                    ...
                    year =
                      case Regex.run(~r/\b(19\d{2}|20\d{2})\b/, trimmed) do
                        [_, year_str] -> year_str
                        _ -> nil
                      end
  ...
  defp extract_movie_metadata_lister(item) do
    ...
    # Extract runtime
    runtime =
      case Floki.find(item, ".runtime") do
        [] -> nil
        elements -> Floki.text(elements) |> String.trim()
      end
    ...
    certificate =
      case Floki.find(item, ".certificate") do
        [] -> nil
        elements -> Floki.text(elements) |> String.trim()
      end
    ...
    # Extract metascore
    metascore =
      case Floki.find(item, ".metascore") do
        [] -> nil
        elements -> Floki.text(elements) |> String.trim()
      end
    ...
    rating =
      case Floki.find(rating_bar, "strong") do
        [] -> nil
        elements -> Floki.text(elements) |> String.trim()
      end
    ...
    # Extract vote count
    votes =
      case Floki.find(item, ".text-muted:last-child") do
        [] ->
          nil
  ...
  defp extract_movie_metadata_ipc(item) do
    ...
    # Extract IMDb rating
    imdb_rating =
      case Floki.find(item, "[data-testid='ratingGroup'] span") do
        [] ->
          nil
  ...
  defp extract_credits_lister(item) do
    ...
    # Extract director
    director =
      case Regex.run(~r/Director?s?:\s*([^|]+)/, credits_text) do
    ...
    # Extract stars
    stars =
      case Regex.run(~r/Stars?:\s*(.+)/, credits_text) do
  ...
  defp process_canonical_movies(movie_data, list_config) do
    ...
    discovered_awards =
      if get_in(list_config.metadata, ["tracks_awards"]) == true do
    ...
    metadata_base =
      ...
      |> Map.merge(list_config.metadata || %{})
    ...
    Logger.info(
      "Processing complete for #{list_config.name}: #{created} created, #{updated} updated, #{queued} queued, #{skipped} skipped, #{errors} errors"
    )
  ...
  defp discover_awards_from_movies(movie_data, list_config) do
    ...
    all_awards =
      ...
      |> Enum.filter(fn movie ->
        Map.has_key?(movie, :extracted_awards) && movie.extracted_awards != []
      end)
    ...
    award_stats =
      ...
      |> Enum.reduce(%{}, fn award, acc ->
        ...
        updated_years =
          if award_year && award_year not in existing.years do
        ...
        updated_categories =
          if award.award_category && award.award_category not in existing.categories do
  ...
  defp process_canonical_movie(movie_data, source_key, metadata_base) do
    ...
    Logger.info(
      "     - raw_description: #{if Map.get(movie_data, :raw_description), do: "YES", else: "NO"}"
    )

    Logger.info(
      "     - award_text: #{if Map.get(movie_data, :award_text), do: "YES - '#{Map.get(movie_data, :award_text)}'", else: "NO"}"
    )

    Logger.info(
      "     - extracted_awards: #{length(Map.get(movie_data, :extracted_awards, []))} items"
    )
  ...
  defp queue_canonical_movie_import(movie_data, source_key, metadata) do
    ...
    Logger.info(
      "   Job args canonical_sources.#{source_key} keys: #{inspect(get_in(job_args, ["canonical_sources", source_key]) |> Map.keys())}"
    )
  ...
  defp update_canonical_sources(movie, source_key, metadata) do
    ...
    Logger.info(
      "   Updated canonical_sources.#{source_key} keys: #{inspect(get_in(updated_sources, [source_key]) |> Map.keys())}"
    )
    ...
    case movie
      ...
      {:ok, updated_movie} ->
        ...
        if stored_metadata do
          Logger.info(
            "   Verification: stored metadata has extracted_awards: #{Map.has_key?(stored_metadata, "extracted_awards")}"
          )
        end
      ...
      {:error, changeset} ->
        Logger.error(
          "❌ Failed to update canonical sources for #{movie.title}: #{inspect(changeset.errors)}"
        )
