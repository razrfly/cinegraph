defmodule Cinegraph.Scrapers.ImdbOscarScraper do
  @moduledoc """
  Scraper for IMDb's Academy Awards pages to get IMDb IDs.
  Based on oscar_data's scrape_imdb_html.py approach.
  
  IMDb stores Oscar data with direct IMDb IDs for films and people,
  making it the most reliable source for matching.
  """
  ...
  defp extract_nominations(next_data, ceremony_year) do
    ...
    try do
      ...
      if awards do
        ...
        Logger.error("Available keys in pageProps: #{inspect(get_in(next_data, ["props", "pageProps"]) |> Map.keys())}")
  ...
  def enhance_ceremony_with_imdb(ceremony) do
    ...
    case fetch_ceremony_imdb_data(film_year) do
      ...
      {:error, reason} ->
        Logger.error("Failed to fetch IMDb data for ceremony #{ceremony.year} (film year #{film_year}): #{reason}")
        {:error, reason}
    end
  end
  ...
  defp enhance_nominees(nominees, imdb_nominations) when is_list(nominees) and is_list(imdb_nominations) do
    nominees
    |> Enum.map(fn nominee ->
  ...
  defp person_matches?(person_name, imdb_people) when is_binary(person_name) and is_list(imdb_people) do
