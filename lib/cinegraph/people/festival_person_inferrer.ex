defmodule Cinegraph.People.FestivalPersonInferrer do
  ...
  def infer_all_director_nominations do
    ...
    Logger.info(
      "Director inference complete: #{success_count} linked, #{skipped_count} skipped (not directors), #{failed_count} failed"
    )
  ...
  def get_inference_stats do
    ...
    %{
      director_categories:
        Enum.map(director_categories, fn {_id, name, org} ->
          "#{org}: #{name}"
        end),
