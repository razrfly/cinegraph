defmodule Cinegraph.Workers.CanonicalPageWorker do
    ...
    case ImdbCanonicalScraper.fetch_single_page(list_id, page, tracks_awards) do
      {:ok, movies} ->
        ...
        if length(movies) > 0 do
          ...
          Logger.info(
            "Data type check - is_map: #{is_map(first_movie)}, is_list: #{is_list(movies)}"
          )
        else
        ...
        Logger.info(
          "Page #{page} complete - Created: #{created}, Updated: #{updated}, Queued: #{queued}, Skipped: #{skipped}"
        )
      ...
      case Movies.get_movie_by_imdb_id(imdb_id) do
        nil ->
          ...
          job_args = %{
            ...
            # Add this for the unique constraint
            "source_key" => source_key
          }
  ...
  defp broadcast_page_progress(list_key, page, total_pages, status, extra_data \\ %{}) do
    ...
    Phoenix.PubSub.broadcast(
      ...
      "import_progress",
      {:canonical_progress,
       Map.merge(Map.merge(base_data, extra_data), %{
