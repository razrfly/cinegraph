defmodule Cinegraph.Workers.PredictionsWorker do
  ...
  def perform(%Oban.Job{
    ...
    Logger.info(
      "Successfully cached #{map_size(movie_scores)} predictions for decade #{decade}, profile #{profile_id}"
    )
  ...
  def perform(%Oban.Job{args: %{"action" => "aggregate_validation", "profile_id" => profile_id}}) do
    ...
    case PredictionCache.get_cached_predictions(2020, profile_id) do
      ...
      db_cache ->
        updated_metadata =
          Map.put(db_cache.metadata || %{}, "validation_data", aggregated_validation)
        ...
        Cachex.put(:predictions_cache, validation_cache_key, aggregated_validation,
          ttl: :timer.hours(24)
        )

        Logger.info(
          "Aggregated and cached validation data, overall accuracy: #{overall_accuracy}%"
        )
    end
  ...
  def perform(%Oban.Job{args: %{"action" => "calculate_comparison", "profile_id" => _profile_id}}) do
    ...
    comparison_data =
      Enum.map(profiles, fn profile ->
        ...
        overall_accuracy =
          if map_size(decade_accuracies) > 0 do
    ...
    case PredictionCache.get_cached_predictions(2020, default_profile.id) do
      ...
      db_cache ->
        updated_metadata =
          Map.put(db_cache.metadata || %{}, "profile_comparison", profile_comparison)
