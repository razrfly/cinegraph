defmodule Cinegraph.Workers.OscarImportWorker do
      ...
      {:ok, result} ->
        case result do
          ...
          %{movies_created: _} ->
            ...
            Logger.info(
              "Completed Oscar import for #{year}: #{result.total_nominees} nominees processed"
            )
          ...
          _ ->
            ...
            Logger.warning(
              "Unexpected result format from Cultural.import_oscar_year: #{inspect(result)}"
            )
  ...
  def perform(
        %Oban.Job{
          args: %{"action" => "import_range", "start_year" => start_year, "end_year" => end_year}
        } = job
      ) do
    ...
    case Cultural.import_oscar_years(start_year..end_year, create_movies: true) do
      ...
      results when is_map(results) ->
        ...
        total_created =
          Enum.reduce(results, 0, fn {_year, result}, acc ->
            case result do
