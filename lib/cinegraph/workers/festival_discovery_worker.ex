defmodule Cinegraph.Workers.FestivalDiscoveryWorker do
  ...
  @credit_based_confidence_threshold 0.85  # 85% minimum similarity for credit matches
  @credit_based_confidence_gap 0.10        # 10% gap required between best and second-best matches

  # TMDb person search thresholds  
  @tmdb_confidence_threshold 0.80           # 80% minimum similarity for TMDb matches
  @tmdb_confidence_gap 0.15                 # 15% gap required between best and second-best matches
  @tmdb_imdb_bonus 0.30                     # 30% bonus for exact IMDb ID matches
  @tmdb_popularity_bonus 0.05               # 5% bonus for popular people (popularity > 10)

  # Performance and quality settings
  @person_linking_timeout 5_000             # 5 second timeout for person linking operations
  ...
  def perform(%Oban.Job{args: %{"ceremony_id" => ceremony_id}} = job) do
    case Repo.get(FestivalCeremony, ceremony_id) |> Repo.preload(:organization) do
      ...
      ceremony ->
        ...
        person_linking_summary = track_ceremony_person_linking_metrics(ceremony, categories, results)
  ...
  defp find_or_create_person(person_imdb_ids, nominee_name, category, movie) do
    ...
    ApiTracker.track_lookup("person_linking", "find_or_create_person", primary_imdb_id, fn ->
  ...
  defp find_person_by_credits(nominee_name, person_imdb_ids, category, movie) when is_binary(nominee_name) do
    ...
    if String.trim(nominee_name) == "" do
      ...
      best_match = find_best_person_match(people_with_credits, normalized_nominee_name, person_imdb_ids)
      ...
      case best_match do
        %{id: person_id, name: matched_name, confidence: confidence} ->
          ...
          ApiTracker.track_lookup("person_linking", "credit_based_match", nominee_name, fn ->
          ...
          Logger.info("Credit-based person linking successful: '#{nominee_name}' → '#{matched_name}' (confidence: #{Float.round(confidence, 3)}, person_id: #{person_id}, category: #{category && category.name || "unknown"})")
          person_id
          
        nil ->
          # Track failed credit-based linking
          ApiTracker.track_lookup("person_linking", "credit_based_no_match", nominee_name, fn ->
          ...
          Logger.debug("No suitable person match found via credits for: #{nominee_name} (category: #{category && category.name || "unknown"}, movie: #{movie && movie.title || "unknown"})")
          nil
      end
    end
  end
      ...
      |> Enum.map(fn person ->
        ...
        imdb_bonus = 
          if person.imdb_id && person.imdb_id in person_imdb_ids do
            0.2  # 20% bonus for IMDb ID match
          else
            0.0
          end
    ...
    case scored_matches do
      ...
      [best_match | rest] ->
        ...
        case rest do
          ...
          [second_best | _] when best_match.confidence - second_best.confidence > @credit_based_confidence_gap ->
            # Clear winner (10% confidence gap)
            best_match
            
          _ ->
            # Multiple similar matches - too ambiguous
            Logger.debug("Multiple similar person matches found for '#{normalized_nominee_name}' - skipping ambiguous match")
            nil
        end
    end
  end
  ...
  defp track_ceremony_person_linking_metrics(ceremony, categories, results) do
    # Count person categories vs film categories
    person_categories = 
      Enum.filter(categories, fn category_data ->
        category_name = extract_category_name(category_data, :oscar_format)
        category_name && String.contains?(String.downcase(category_name), ["actor", "actress", "director", "writing", "cinematography"])
      end)
    ...
    ApiTracker.track_lookup("person_linking", "ceremony_summary", "#{ceremony.organization.abbreviation}_#{ceremony.year}", fn ->
      {:ok, %{
      ...
      "person_categories_ratio" => (if length(categories) > 0, do: person_category_count / length(categories), else: 0.0)
    })
  ...
  defp build_enhanced_credit_query(category, movie) do
    base_query = 
      from p in Person,
        join: c in Credit, on: c.person_id == p.id,
  ...
  defp find_or_create_person_via_tmdb(person_imdb_ids, nominee_name) when is_binary(nominee_name) do
    # Skip if no nominee name provided
    if String.trim(nominee_name) == "" do
      ...
      ApiTracker.track_lookup("person_linking", "tmdb_person_search", nominee_name, fn ->
        case search_tmdb_person(nominee_name, person_imdb_ids) do
          {:ok, tmdb_person_data} ->
            ...
            case create_person_from_tmdb_data(tmdb_person_data, nominee_name) do
              nil -> {:error, :tmdb_person_creation_failed}
              person_id -> {:ok, %{strategy: "tmdb_search", person_id: person_id, tmdb_id: tmdb_person_data["id"]}}
            end
  ...
  defp search_tmdb_person(nominee_name, person_imdb_ids) do
    ...
    case Task.await(task, @person_linking_timeout) do
      {:ok, %{"results" => results}} when results != [] ->
        ...
        case find_best_tmdb_person_match(results, nominee_name, person_imdb_ids) do
          {:ok, person_data} ->
            Logger.info("TMDb person match found: #{person_data["name"]} (ID: #{person_data["id"]})")
            {:ok, person_data}
    ...
    error ->
      Logger.warning("TMDb person search timeout or error for '#{nominee_name}': #{inspect(error)}")
      {:error, :timeout}
  end
  ...
  defp create_minimal_person_record(person_imdb_ids, nominee_name) do
    ...
    if primary_imdb_id do
      ...
      case %Person{} |> Person.imdb_changeset(attrs) |> Repo.insert() do
        {:ok, person} ->
          Logger.info("Successfully created minimal person record: #{person.name} (ID: #{person.id})")
          person.id
      ...
      Logger.debug("No IMDb ID provided, cannot create minimal person record for: #{nominee_name}")
      nil
    end
  end
end
