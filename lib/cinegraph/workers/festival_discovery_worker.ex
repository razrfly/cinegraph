defmodule Cinegraph.Workers.FestivalDiscoveryWorker do
  ...
  def perform(%Oban.Job{args: %{"ceremony_id" => ceremony_id}} = job) do
    case Repo.get(FestivalCeremony, ceremony_id) |> Repo.preload(:organization) do
      ...
      ceremony ->
        Logger.info("Processing #{ceremony.organization.abbreviation} ceremony #{ceremony.year} (ID: #{ceremony.id})")
        
        # First ensure ceremony is enhanced with IMDb data
        ceremony = ensure_imdb_enhancement(ceremony)
        
        # Process each category
        categories = ceremony.data["categories"] || ceremony.data[:categories] || []
        ...
        |> Enum.flat_map(fn {category_data, index} ->
          ...
          Logger.info("Processing category #{index + 1}/#{length(categories)}: #{category_name || "Unknown"}")
          
          # Get or create the Oscar category
          oscar_category = ensure_category_exists(category_name, ceremony.organization_id)
          
          # Process nominees if category was created successfully
          category_results = if oscar_category do
            process_category(category_data, oscar_category, ceremony)
          else
  ...
  defp ensure_category_exists(category_name, organization_id) do
    case Festivals.get_category(organization_id, category_name) do
      nil ->
        ...
        Logger.info("Creating new festival category: #{category_name} for organization #{organization_id}")
  ...
  defp process_category(category_data, fest_category, ceremony) do
    nominees = category_data["nominees"] || category_data[:nominees] || []
  ...
  defp process_nominee(nominee, category, ceremony) do
    ...
    Logger.info("Processing nominee: #{film_title} (IMDb: #{film_imdb_id || "none"}) in #{category_name} for #{ceremony.year}")
    ...
    if String.contains?(category_name, ["Music (Original Song)", "Original Song"]) do
      ...
      cond do
        ...
        !is_nil(film_imdb_id) ->
          ...
          movie_result = ensure_movie_exists(film_imdb_id, film_title, nominee)
  ...
  defp ensure_movie_exists(imdb_id, film_title, nominee) do
    case Repo.get_by(Movie, imdb_id: imdb_id) do
      nil ->
        # Movie doesn't exist, queue TMDbDetailsWorker
        Logger.info("Movie not found for IMDb ID #{imdb_id} (#{film_title}), queuing creation job")
  ...
  defp create_nomination(movie, nominee, category, ceremony) do
    ...
    existing_count = from(n in FestivalNomination,
    ...
    if existing_count > 0 do
      Logger.debug("Nomination already exists for #{movie.title} in #{category.name} (found #{existing_count})")
  ...
  defp find_or_create_person(person_imdb_ids, _nominee_name) do
    ...
    person = Enum.find_value(person_imdb_ids, fn imdb_id ->
