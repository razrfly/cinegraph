defmodule Cinegraph.Workers.HealthCacheWarmer do
  ...
  Without this worker, the first request after deploy or after the 35-minute
  ...
  Strategy: re-run `Cinegraph.Health.Facade.compute_full_verdict/0` every 30
  minutes. Drift checks share `:health_cache` (35-min Cachex TTL), so warming
  every 30 min means each row is re-computed before it expires. Net effect:
