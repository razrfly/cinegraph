defmodule Cinegraph.Metrics.PQSMonitoring do
  ...
  def get_coverage_metrics do
    ...
    # People with ≥3 credits
    eligible_people = count_eligible_people()
    ...
    %{
      ...
      director_coverage_percent:
        safe_percentage(directors_with_pqs.with_scores, directors_with_pqs.total),
      ...
      people_without_pqs_percent:
        safe_percentage(eligible_people - people_with_pqs, eligible_people)
    }
  end
  ...
  def get_performance_metrics do
    # Last 24 hours
    recent_jobs = get_recent_pqs_jobs(24)
  ...
  defp get_average_score_age do
    ...
    case result do
      nil -> 0.0
      # Convert to days
      %Decimal{} = decimal -> Float.round(Decimal.to_float(decimal) / 86400, 1)
      # Convert to days
      seconds when is_number(seconds) -> Float.round(seconds / 86400, 1)
      _ -> 0.0
    end
  end
  ...
  defp get_recent_pqs_jobs(hours_back) do
    ...
    Repo.all(
      from j in "oban_jobs",
        where:
          j.worker == "Cinegraph.Workers.PersonQualityScoreWorker" and j.inserted_at >= ^cutoff,
        select: %{
          id: j.id,
          state: j.state,
  ...
  defp calculate_throughput(jobs) do
    if length(jobs) > 0 do
      # Jobs per hour over 24 hours
      Float.round(length(jobs) / 24, 1)
    else
      0
    end
  end
  ...
  defp get_current_queue_depth do
    Repo.one(
      from j in "oban_jobs",
        where:
          j.worker == "Cinegraph.Workers.PersonQualityScoreWorker" and
    ...
    result =
      Repo.one(
        from mc in "movie_credits",
          ...
          left_join: pm in "person_metrics",
  ...
  defp get_last_successful_batch do
    Repo.one(
      from j in "oban_jobs",
        where:
          j.worker == "Cinegraph.Workers.PersonQualityScoreWorker" and j.state == "completed",
  ...
  defp determine_health_status(coverage, freshness, performance) do
    cond do
      coverage.coverage_percent >= 95.0 and freshness.fresh_percent >= 90.0 and
          performance.failure_rate_percent <= 1.0 ->
        :healthy

      coverage.coverage_percent >= 80.0 and freshness.fresh_percent >= 70.0 and
          performance.failure_rate_percent <= 5.0 ->
        :warning
  ...
  defp generate_system_alerts(coverage, freshness, performance) do
    ...
    alerts =
      if coverage.coverage_percent < 95.0 do
    ...
    alerts =
      if freshness.fresh_percent < 90.0 do
    ...
    alerts =
      if performance.failure_rate_percent > 1.0 do
