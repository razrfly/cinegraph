defmodule Cinegraph.Metrics.MetricScore do
  ...
  schema "metric_scores" do
    ...
    # Category scores
    # Normalized aggregate of all ratings
    field :ratings_score, :float
    # Normalized aggregate of all awards
    field :awards_score, :float
    # Normalized aggregate of financial metrics
    field :financial_score, :float
    # Normalized aggregate of cultural list inclusions
    field :cultural_score, :float

    # Total weighted score
    field :total_score, :float

    # Raw metric values used (for transparency)
    # {"imdb_rating": 8.3, "oscar_wins": 7, ...}
    field :metric_values, :map

    # Metadata
    # Where this movie ranks in the distribution
    field :percentile_rank, :float
    field :calculated_at, :utc_datetime
    # Number of metrics available for this movie
    field :metrics_available, :integer
    # List of missing metric codes
    field :metrics_missing, {:array, :string}
  ...
  defp validate_category_scores(changeset) do
    changeset
    |> validate_number(:ratings_score,
      greater_than_or_equal_to: 0.0,
      less_than_or_equal_to: 1.0,
      allow_nil: true
    )
    |> validate_number(:awards_score,
      greater_than_or_equal_to: 0.0,
      less_than_or_equal_to: 1.0,
      allow_nil: true
    )
    |> validate_number(:financial_score,
      greater_than_or_equal_to: 0.0,
      less_than_or_equal_to: 1.0,
      allow_nil: true
    )
    |> validate_number(:cultural_score,
      greater_than_or_equal_to: 0.0,
      less_than_or_equal_to: 1.0,
      allow_nil: true
    )
    |> validate_number(:percentile_rank,
      greater_than_or_equal_to: 0.0,
