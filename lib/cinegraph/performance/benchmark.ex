defmodule Cinegraph.Performance.Benchmark do
  ...
  def benchmark_predictions_page do
    ...
    IO.puts(
      "  Predictions: #{results.predictions.time_ms}ms (#{results.predictions.count} movies)"
    )

    IO.puts(
      "  Validation: #{results.validation.time_ms}ms (#{results.validation.decade_count} decades)"
    )
