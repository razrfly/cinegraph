defmodule Cinegraph.Movies.Filters do
  ...
  def apply_sorting(query, params) do
    case params["sort"] do
      ...
      "runtime" -> order_by(query, [m], asc: m.runtime)
      "runtime_desc" -> order_by(query, [m], desc: m.runtime)
      "date_added" -> order_by(query, [m], asc: m.inserted_at)
      "date_added_desc" -> order_by(query, [m], desc: m.inserted_at)
