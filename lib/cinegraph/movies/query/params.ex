defmodule Cinegraph.Movies.Query.Params do
  ...
  embedded_schema do
    ...
    field :festival_id, :integer  # kept for backwards compatibility
    field :festivals, {:array, :integer}, default: []  # new multi-select field
  ...
  def changeset(params) do
    ...
    |> validate_inclusion(:rating_preset, @valid_rating_presets)
    |> validate_inclusion(:discovery_preset, @valid_discovery_presets)
    |> validate_inclusion(:award_preset, @valid_award_presets)
    |> validate_inclusion(:award_status, @valid_award_statuses)
    |> validate_inclusion(:people_role, @valid_people_roles)
  ...
  defp normalize_array_params(params) do
    ...
    Enum.reduce([:genres, :countries, :languages, :lists, :festivals, :people_ids], params, fn key, acc ->
  ...
  defp normalize_people_search(params) do
    case params do
      ...
      %{"people_search[people_ids]" => ids} ->
        role = params["people_search[role_filter]"]
        params
        |> Map.put("people_ids", parse_list_param(ids))
        |> Map.put("people_role", role)
  ...
  defp build_flop_filters(params) do
    ...
    # Add search filter
    filters = if params.search && params.search != "" do
      filters ++ [%{field: :title, op: :ilike_and, value: params.search}]
    else
      filters
    end
    
    # Add simple field filters that Flop can handle directly
    filters = if params.show_unreleased == false do
