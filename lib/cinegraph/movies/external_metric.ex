defmodule Cinegraph.Movies.ExternalMetric do
  ...
  def from_tmdb(movie_id, tmdb_data) do
    ...
    # Ratings
    metrics =
      if tmdb_data["vote_average"] && tmdb_data["vote_average"] > 0 do
        [
          %{
            movie_id: movie_id,
            source: "tmdb",
    ...
    metrics =
      if tmdb_data["vote_count"] && tmdb_data["vote_count"] > 0 do
        [
          %{
            movie_id: movie_id,
            source: "tmdb",
    ...
    # Financial
    metrics =
      if tmdb_data["budget"] && tmdb_data["budget"] > 0 do
        [
          %{
            movie_id: movie_id,
            source: "tmdb",
    ...
    metrics =
      if tmdb_data["revenue"] && tmdb_data["revenue"] > 0 do
        [
          %{
            movie_id: movie_id,
            source: "tmdb",
  ...
  def from_omdb(movie_id, omdb_data) do
    ...
    # Awards
    metrics =
      if omdb_data["Awards"] && omdb_data["Awards"] != "N/A" do
  ...
  defp parse_awards_text(text) do
    ...
    # Parse Oscar wins
    metadata =
      case Regex.run(~r/Won (\d+) Oscar/, text) do
    ...
    # Parse total wins
    metadata =
      case Regex.run(~r/(\d+) wins?/, text) do
    ...
    metadata =
      case Regex.run(~r/(\d+) nominations?/, text) do
