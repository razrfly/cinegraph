defmodule Cinegraph.Collaborations.PathFinder do
  ...
  defp get_connected_people(person_id) do
    ...
    query = from mc1 in Credit,
      join: mc2 in Credit, on: mc1.movie_id == mc2.movie_id,
  ...
  defp find_connecting_movie(person_a_id, person_b_id) do
    Repo.one(
      from m in "movies",
      join: mc1 in Credit, on: mc1.movie_id == m.id,
      join: mc2 in Credit, on: mc2.movie_id == m.id,
      where: mc1.person_id == ^person_a_id,
      where: mc2.person_id == ^person_b_id,
      select: %{id: m.id, title: m.title, year: fragment("EXTRACT(YEAR FROM ?)", m.release_date)},
      limit: 1
    )
  end
  ...
  def precalculate_popular_paths(limit \\ 100) do
    ...
    popular_people = Repo.all(
      from p in Person,
      join: mc in Credit, on: mc.person_id == p.id,
      group_by: p.id,
