    ...
    query =
      from c in Collaboration,
        ...
        select: %{
          ...
          high_genre_diversity:
            sum(fragment("CASE WHEN ? > 0.7 THEN 1 ELSE 0 END", c.genre_diversity_score)),
          high_role_diversity:
            sum(fragment("CASE WHEN ? > 0.7 THEN 1 ELSE 0 END", c.role_diversity_score))
        }
