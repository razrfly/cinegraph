...
| `mix predictions.list [--json]` | The display board (CLI twin of `/algorithms`): per displayable list — members, served model, frontier cutoff, grade, recall LB. Reads `MovieLists.list_with_model_stats/0`. |
| `mix predictions.candidates LIST [--mode predictions\|candidates\|members\|all] [--limit N] [--min-sources N] [--min-votes N] [--json]` | Ranked films under the served model (CLI twin of the show page's Predictions tab). Frontier-gated in `predictions` mode; eligibility is evidence-based (≥2 distinct metric sources by default, #1078 §0 — never a single metric's value); per-row probability only when the honesty gate passes (`ProbabilityCalibration.informative?/1` — increasing Platt + grade ≠ insufficient). Delegates to `Cinegraph.Predictions.Candidates`. |
| `mix predictions.recalibrate [--only L1,L2] [--commit]` | Refit served models' Platt calibration (#1074: two-stage balanced fit) from **reproduced** holdout pairs (`Trainer.holdout_pairs/1`) — no retraining, no holdout re-spend, no grade change. Refuses on reproduction mismatch or Brier regression vs identity. Dry-run by default. |
| `mix predictions.model.export [--list L\|--all]` | Export served model(s) as reviewable ~5KB JSON bundles → `priv/prediction_models/` (#1043). Deterministic encoding — commit them. |
| `mix predictions.model.import FILE` | Prod-side import: idempotent, transactional; Gate 1 substrate parity (refuse, never silently activate) + Gate 2 verbatim holdout measurement. |
| `mix predictions.model.push [--list L\|--all] [--check\|--commit]` | Studio-side promotion via `ProdRpc`. `--check` = read-only prod substrate preflight (works on any image age); `--commit` ships + imports + activates. See MAINTENANCE.md for the full recipe. |
