...
IO.puts("\n📈 Final State:")
IO.puts("  • 1001 Movies marked: #{final_count} (#{if final_count > initial_count, do: "+#{final_count - initial_count}", else: "no change"})")
IO.puts("  • Total movies in DB: #{final_total} (#{if final_total > initial_total, do: "+#{final_total - initial_total}", else: "no change"})")

# Check Oban jobs
oban_jobs = from(j in Oban.Job, 
