...
sample_collabs = Repo.all(
  from c in Cinegraph.Collaborations.Collaboration,
  join: p1 in Cinegraph.Movies.Person, on: p1.id == c.person_a_id,
  join: p2 in Cinegraph.Movies.Person, on: p2.id == c.person_b_id,
...
Enum.each(sample_collabs, fn collab ->
  rating = if collab.avg_rating, do: Float.round(Decimal.to_float(collab.avg_rating), 1), else: "N/A"
...
details = Repo.all(
  ...
  join: c in Cinegraph.Collaborations.Collaboration, on: c.id == cd.collaboration_id,
  join: m in Cinegraph.Movies.Movie, on: m.id == cd.movie_id,
  join: p1 in Cinegraph.Movies.Person, on: p1.id == c.person_a_id,
  join: p2 in Cinegraph.Movies.Person, on: p2.id == c.person_b_id,
...
orphaned_details = Repo.one(
  from cd in Cinegraph.Collaborations.CollaborationDetail,
  left_join: c in Cinegraph.Collaborations.Collaboration, on: c.id == cd.collaboration_id,
  where: is_nil(c.id),
  select: count(cd.id)
)
...
missing_movies = Repo.one(
  from cd in Cinegraph.Collaborations.CollaborationDetail,
  left_join: m in Cinegraph.Movies.Movie, on: m.id == cd.movie_id,
  where: is_nil(m.id),
  select: count(cd.id)
)
...
IO.puts("\nRevenue Data:")
IO.puts("  Collaborations with revenue: #{revenue_stats.with_revenue}/#{revenue_stats.total_collabs}")
avg_rev = if revenue_stats.avg_revenue, do: div(trunc(Decimal.to_float(revenue_stats.avg_revenue)), 1_000_000), else: 0
