...
if length(movies) > 0 do
  ...
  valid_results = Enum.filter(batch_results, fn r -> 
  ...
  if length(valid_results) == length(batch_results) do
    ...
    IO.puts("   ❌ Batch scoring has issues: #{length(valid_results)}/#{length(batch_results)} valid")
  end
  ...
  if length(result.predictions) > 0 do
    ...
    invalid = Enum.filter(result.predictions, fn p ->
